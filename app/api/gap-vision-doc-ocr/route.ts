...
// AI gap: Vision-Based Document/Contract OCR (salesforce)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'salesforce Gap vision-doc-ocr' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You extract structured data from contract uploads (images/PDFs).";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'vision-doc-ocr', payload: { keys: Object.keys(body) } });
    return NextResponse.json({ extraction: out });
  } catch (e: any) {
...
export async function GET() {
  return NextResponse.json({ feature: 'vision-doc-ocr', events: gapFeatures.filter(r => r.slug === 'vision-doc-ocr').length });
}
