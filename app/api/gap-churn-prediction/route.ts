...
// AI gap: Churn Prediction Model (salesforce)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'salesforce Gap churn-prediction' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You predict customer churn risk using account, opportunity, and engagement data.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'churn-prediction', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'churn-prediction', events: gapFeatures.filter(r => r.slug === 'churn-prediction').length });
}
