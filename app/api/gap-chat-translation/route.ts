...
// AI gap: Conversation Translation (salesforce)
...
async function callLLM(systemPrompt: string, userMsg: string, maxTokens = 1400) {
  ...
  const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + OPENROUTER_API_KEY, 'Content-Type': 'application/json', 'HTTP-Referer': 'http://localhost:3000', 'X-Title': 'salesforce Gap chat-translation' },
...
export async function POST(req: NextRequest) {
  try {
    ...
    const sys = "You translate live-chat conversations preserving CRM terminology.";
    const user = `Body: ${JSON.stringify(body).slice(0, 4000)}`;
    const out = await callLLM(sys, user);
    gapFeatures.push({ at: new Date().toISOString(), slug: 'chat-translation', payload: { keys: Object.keys(body) } });
...
export async function GET() {
  return NextResponse.json({ feature: 'chat-translation', events: gapFeatures.filter(r => r.slug === 'chat-translation').length });
}
