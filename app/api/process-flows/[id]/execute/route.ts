...
async function executeWorkflow(
  ...
  const execution = await prisma.processFlowExecution.create({
    data: {
  ...
  try {
    ...
    await prisma.processFlowExecution.update({
      where: { id: execution.id },
      data: {
        status: 'COMPLETED',
        completedAt: new Date(),
        executionLog: { steps: executionLog },
      },
    });

    return { success: true, executionId: execution.id, log: executionLog };
  } catch (error: any) {
    await prisma.processFlowExecution.update({
      where: { id: execution.id },
      data: {
...
async function createTaskAction(data: any, objectId: string, objectType: string, log: any[], userId: string) {
  ...
  await prisma.task.create({
    data: {
      subject: title, // Task model uses 'subject' instead of 'title'
      description,
      dueDate: dueDate ? new Date(dueDate) : null,
      status: 'NOT_STARTED', // Correct enum value from TaskStatus
      priority: 'MEDIUM',
      assignedTo: assignedTo || userId, // Use provided assignee or default to current user
      createdBy: userId, // Set the creator to the current user
      relatedTo: objectId, // Use relatedTo field to store the related object ID
    },
  });
...
async function apiCallAction(data: any, objectId: string, objectType: string, log: any[]) {
  ...
  try {
    ...
    if (object) {
      // Replace field variables like {{field.name}}
    ...
    let processedHeaders: any = {
      ...
      // Add internal workflow auth header for localhost calls
    ...
    if (body && typeof body === 'string') {
      ...
      if (object) {
        Object.keys(object).forEach((key) => {
          const regex = new RegExp(`\\{\\{field\\.${key}\\}\\}`, 'g');
          // Get the value and escape special JSON characters for string values
          const value = object[key];
          let replacement = '';
          if (value === null || value === undefined) {
            replacement = '';
          } else if (typeof value === 'string') {
            // Escape special characters for JSON string context (quotes, backslashes, newlines, etc.)
            replacement = value
              .replace(/\\/g, '\\\\')
              .replace(/"/g, '\\"')
              .replace(/\n/g, '\\n')
