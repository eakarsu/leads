...
// Non-AI gap: Payment Processor for Invoices (salesforce)
...
export async function POST(req: NextRequest) {
  try {
    ...
    gapFeatures.push({ at: record.createdAt, slug: 'payment-processor', payload: record });
...
export async function GET() {
  return NextResponse.json({ feature: 'payment-processor', events: gapFeatures.filter(r => r.slug === 'payment-processor').length });
}
