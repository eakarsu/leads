  ...
  const handleGenerateBrief = async () => {
    ...
    try {
      const response = await fetch('/api/ai/campaign-brief', {
        ...
        body: JSON.stringify({
          industry: (campaign?.client as any)?.industry || 'Technology',
          icp: (campaign as any)?.targetPersona || 'Enterprise decision makers',
          offer: campaign?.description || 'Lead generation service',
          channels: (campaign as any)?.channel || 'Email, LinkedIn',
          geo: (campaign as any)?.targetRegions || 'North America',
          tone: 'Professional',
        }),
      });
